...
def init_db():
    ...
    ''')
    # 🔑 Authorized/Sudo Users ဇယားသစ် (Owner မှ ခွင့်ပြုထားသူများ)
    cursor.execute('''
...
# 🔑 Authorized/Authorized Sudo Check
...
def handle_all_messages(message):
    ...
    # GROUPS
        ...
        # Anti-Link Check (Authorized/Sudo users are immune)
        if not is_authorized(message.from_user.id):
    ...
    if text.startswith('/start') or text.startswith('!start'):
        if is_authorized(message.from_user.id):
            bot.reply_to(message, "👋 မင်္ဂလာပါ Authorized Admin! အကူအညီကြည့်ရန် /help ကို နှိပ်ပါ။")
            return
